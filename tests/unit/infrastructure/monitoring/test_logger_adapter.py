...
class TestLoggerAdapter:
    async def test_debug(
        ...
        stdlib_logger.debug.assert_called_once_with("debug msg")

    async def test_info(self, adapter: LoggerAdapter, stdlib_logger: MagicMock) -> None:
        await adapter.info("info msg")
        stdlib_logger.info.assert_called_once_with("info msg")

    async def test_warning(
        self, adapter: LoggerAdapter, stdlib_logger: MagicMock
    ) -> None:
        await adapter.warning("warn msg")
        stdlib_logger.warning.assert_called_once_with("warn msg")

    async def test_error(
        self, adapter: LoggerAdapter, stdlib_logger: MagicMock
    ) -> None:
        await adapter.error("error msg")
        stdlib_logger.error.assert_called_once_with("error msg")

    async def test_error_with_exc_info(
        self, adapter: LoggerAdapter, stdlib_logger: MagicMock
    ) -> None:
        await adapter.error("failed", exc_info=True)
        stdlib_logger.error.assert_called_once_with("failed", exc_info=True)

    async def test_info_with_extra(
        self, adapter: LoggerAdapter, stdlib_logger: MagicMock
    ) -> None:
        await adapter.info("player connected", extra={"player_id": "123"})
