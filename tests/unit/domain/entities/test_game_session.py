...
class TestGameSessionPlayers:
    async def test_connect_player(self) -> None:
        ...
        await session.connect_player(p)
    ...
    async def test_disconnect_player(self) -> None:
        ...
        await session.connect_player(p)
        await session.disconnect_player(pid)
    ...
    async def test_disconnect_unknown_raises(self) -> None:
        ...
        with pytest.raises(PlayerNotConnectedException):
            await session.disconnect_player(make_player_id())


class TestGameSessionSubmitInput:
    async def test_fifo_enqueue(self) -> None:
    ...
    async def test_fifo_multiple_inputs(self) -> None:
        session = make_session(control_mode=ControlMode.FIFO)
        await session.start()
        gi1 = make_game_input(Button.LEFT)
        gi2 = make_game_input(Button.RIGHT)
        await session.submit_input(gi1)
        await session.submit_input(gi2)
        assert session.input_queue.size == 2
        assert session.metrics.total_commands == 2

    async def test_voting_collects_vote(self) -> None:
    ...
    async def test_voting_multiple_votes(self) -> None:
        session = make_session(control_mode=ControlMode.VOTING)
        await session.start()
        pid1 = make_player_id()
        pid2 = make_player_id()
        await session.submit_input(make_game_input(Button.LEFT, pid1))
        await session.submit_input(make_game_input(Button.RIGHT, pid2))
        assert session.current_vote is not None
    ...
    async def test_submit_when_not_running_raises(self) -> None:
        ...
        with pytest.raises(SessionNotRunningException):
            await session.submit_input(make_game_input())


class TestGameSessionControlMode:
    async def test_change_to_voting(self) -> None:
        session = make_session(control_mode=ControlMode.FIFO)
        await session.change_control_mode(ControlMode.VOTING)
    ...
    async def test_change_to_fifo_clears_vote(self) -> None:
        ...
        await session.submit_input(make_game_input())
        assert session.current_vote is not None
        await session.change_control_mode(ControlMode.FIFO)
